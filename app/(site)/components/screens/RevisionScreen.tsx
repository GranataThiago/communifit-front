...
export const RevisionScreen = () => {
    ...
    const { handleSubmit, formState: { errors }, register, control, getValues, setValue, watch } = useForm<RevisionForm>();
                ...
                <div className='pics flex gap-2 flex-wrap'>
                    <label className={`w-full h-64 flex flex-col items-center px-4 py-6 bg-white text-blue rounded-md border-2 border-dashed`}>
                        <span className="mt-auto mb-auto text-gray-300 text-center">
                            {uploadedPics?.length > 0 ? `${uploadedPics.length} pictures uploaded` : 'Upload your photo...'}
                        </span>
                        <input {...register('pics')} onChange={onImagesUploaded} type='file' multiple className='w-100 h-100 opacity-0'/>
                    </label>
